...
DB_HOST = os.environ.get('DB_HOST', 'mongodb-georgiastateparks')
...
DATASET_FILE = 'georgiastateparks.json'
...
class Info(Resource):
    description = {
        'id': 'georgiastateparks',
        'displayName': 'Georgia State Parks',
        'type': 'cluster',
        'center': {'latitude': '32.679219', 'longitude': '-83.280177'},
        'zoom': 5
    }
...
class DataLoad(Resource):
    def get(self):
        ...
        collection = database.georgiastateparks
...
@application.route('/')
def index():
    return 'Welcome to the Georgia State Parks data service.'
